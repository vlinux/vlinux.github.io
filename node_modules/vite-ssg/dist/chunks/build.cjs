...
async function build(cliOptions = {}, viteConfig = {}) {
  ...
  const { renderToString } = format === "esm" ? await import('vue/server-renderer') : _require("vue/server-renderer");
  await Promise.all(routesPaths.map(async (route) => {
    try {
      ...
      renderPreloadLinks(jsdom$1.window.document, ctx.modules || /* @__PURE__ */ new Set(), ssrManifest);
      head?.updateDOM(jsdom$1.window.document);
      const html = jsdom$1.serialize();
